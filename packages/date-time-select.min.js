...
d=a.find('.form-field[class$="year"]'),i=a.find('.form-field[class$="month"]'),f=a.find('.form-field[class$="day"]');a.find('.form-field[class$="year"], .form-field[class$="month"]').change(function(){selectedYear=d.find(".ui-button-text").html();var a=i.find(".ui-button-text").html();selectedMonth=e.inArray(a,",January,February,March,April,May,June,July,August,September,October,November,December".split(","));var a=(new Date(selectedYear,selectedMonth,0)).getDate(),b=f.find("select");selectedDay=
f.find(".ui-button-text").html();f.find("select option").remove();for(dayIndex=1;dayIndex<a;dayIndex++)f.find("select").append('<option value="'+dayIndex+'">'+dayIndex+"</option>");selectedDay>a&&(selectedDay=a);b.selectMenu();k(f,selectedDay)});var c=a.find('.form-field[class$="hour"]'),j=a.find('.form-field[class$="minute"]');b.click(function(a){a.preventDefault();h(d,"year").find("select").trigger("click");h(i,"month").find("select").trigger("click");h(f,"day").find("select").trigger("click");
