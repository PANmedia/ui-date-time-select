...
f.find(".ui-button-text").html();f.find("select option").remove();for(dayIndex=1;dayIndex<a;dayIndex++)f.find("select").append('<option value="'+dayIndex+'">'+dayIndex+"</option>");selectedDay>a&&(selectedDay=a);b.selectMenu();k(f,selectedDay)});var c=a.find('.form-field[class$="_hour"]'),j=a.find('.form-field[class$="_minute"]');b.click(function(a){a.preventDefault();h(d,"year").find("select").trigger("change");h(i,"month").find("select").trigger("change");h(f,"day").find("select").trigger("change");
h(c,"hour").find("select").trigger("change");h(j,"minute").find("select").trigger("change");i.find("select").trigger("change")});e(this).closest("form").submit(function(){var b=function(a){return a.find("select option[selected]").val()},e=b(d),h=b(i),g=b(f),k=b(c),b=b(j),e=Math.round((new Date(e,h-1,g,k,b)).getTime()/1E3);a.find('input[type="hidden"]').val(e);return!0})}})}}})(jQuery);
